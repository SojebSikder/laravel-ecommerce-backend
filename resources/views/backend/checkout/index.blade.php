                    ...
                    <div class="col-md-12 mt-3">
                        <div class="card">
                            <div class="card-header">
                                ...
                                {{-- <a href="{{ route('checkout.create') }}" class="btn btn-sm btn-primary float-end mt-3 mr-4">
                                    View all orders
                                </a> --}}
                                            ...
                                            @foreach ($checkouts as $checkout)
                                                <tr>
                                                    <td>
                                                        {{ $checkout->uuid }}
                                                        <br>
                                                        {{ count($checkout->checkout_items) }} items
                                                    </td>
                                                    <td>
                                                        {{ date('d M Y', strtotime($checkout->created_at)) }} at
                                                        {{ date('h:i a', strtotime($checkout->created_at)) }}
                                                        ({{ $checkout->created_at->diffForHumans() }})
                                                    </td>
                                                    <td>{{ $checkout->fname }} {{ $checkout->lname }}</td>

                                                    <td class="text-center">
                                                        <ul class="table-controls">
                                                            <li>
                                                                <a class="btn btn-sm btn-primary"
                                                                    href="{{ route('checkout.edit', $checkout->id) }}"
                                                            ...
                                                            <li>
                                                                <a class="btn btn-sm btn-danger" href="javascript:void(0);"
                                                                    ...
                                                                    document.getElementById('checkout-delete-{{ $checkout->id }}').submit()
                                                                    }"
                                                                    data-bs-toggle="tooltip" data-bs-placement="top"
                                                                ...
                                                                {{-- delete  --}}
                                                                <form method="post"
                                                                    action="{{ route('checkout.destroy', $checkout->id) }}"
                                                                    id="{{ 'checkout-delete-' . $checkout->id }}">
                                                                    @csrf
                                                                    @method('DELETE')
                                                                </form>
                                                            </li>
